...
import { formatContentFields, filterApplicationsByDate, generateMockApplications } from '$lib/utils/mockData';
...
describe('filterApplicationsByDate', () => {
  ...
  it('should filter by end date inclusive', () => {
    const apps = [
      createApp('2026-05-10T00:00:00.000Z'),
      createApp('2026-05-11T23:59:59.999'), // 本地时间，不带 Z
      createApp('2026-05-12T00:00:00.000Z'),
    ];
    const result = filterApplicationsByDate(apps, undefined, '2026-05-11');
    expect(result).toHaveLength(2);
    const submitted = result.map(a => a.submittedAt);
    expect(submitted).toContain('2026-05-10T00:00:00.000Z');
    expect(submitted).toContain('2026-05-11T23:59:59.999');
  });
...
describe('generateMockApplications', () => {
  it('should generate correct number of applications', () => {
